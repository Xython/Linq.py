...
namespace     = globals()
recurSearcher = set()
Def = AstParser([LiteralParser('def', name='\'def\''),Ref('Name'),CharParser('(', name='\'(\''),Ref('paramList'),CharParser(')', name='\')\''),CharParser(':', name='\':\'')], name = 'Def', toIgnore = [{},{'def',')','(',':'}])
NameIg = AstParser([LiteralParser('[a-zA-Z_][a-zA-Z_0-9]*', name='\'[a-zA-Z_][a-zA-Z_0-9]*\'', isRegex = True)], name = 'NameIg')
param = AstParser([SeqParser([CharParser('*', name='\'*\'')], atmost = 1),Ref('Name'),SeqParser([LiteralParser(':', name='\':\''),Ref('NameIg')], atmost = 1),SeqParser([CharParser('=', name='\'=\''),LiteralParser('None', name='\'None\'')], atmost = 1)], name = 'param', toIgnore = [{NameIg},{'=',':','None','*'}])
